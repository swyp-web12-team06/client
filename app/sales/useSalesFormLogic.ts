...
export function useSalesFormLogic(): SalesFormContextType {
    ...
    useEffect(() => {
        ...
        const fetchModelsAndCategories = async () => {
            setLoading(true);
            try {
                const modelsResponse = await fetch('/api/metadata/ai-models');
                ...
                if (categoriesData.code === 'SUCCESS') {
                    const fetchedCategories = categoriesData.data.map((category: { id: number; name: string }) => ({
                        value: category.id,
                        label: category.name,
                    }));
    ...
    const formatPrompt = (text: string) => {
        ...
        const highlightedText = escapedText.replace(/\[([^\]]*)\]/g, '<span class="text-primary-200">[$1]</span>');
    ...
    return {
        ...
        variables, activeVariable, setActiveVariable,
        variableInfo, setVariableInfo,
