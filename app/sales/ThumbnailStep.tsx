...
export default function ThumbnailStep() {
  const { images, setImages, variables, selectedImageIndex, setSelectedImageIndex } =
    useSalesForm();
  ...
  const handleRoleDrop = (
    ...
    role: 'isRepresentative' | 'isThumbnail1' | 'isThumbnail2',
  ) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  return (
    <div className="space-y-6">
      <div className="flex gap-4">
        {/* 메인 미리보기 영역 */}
        <div
          className="relative flex h-66 min-w-104 items-center justify-center overflow-hidden rounded-[10px] bg-gray-300"
        ...
        <div className="flex w-full flex-col">
          {variables.map((variable) => (
            <Input
              ...
              value={
                (selectedImageIndex !== null &&
                  images[selectedImageIndex]?.optionValues[variable]) ||
                ''
              }
        ...
        <div className="relative">
          ...
          {showPrev && (
            <button
              ...
              className="absolute top-1/2 left-2 z-10 flex h-6 w-6 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full bg-gray-50/50 backdrop-blur-sm"
            >
              <Image
                className="rotate-90"
          ...
          <div
            ...
            className="flex h-26.5 items-center gap-7 overflow-x-auto scroll-smooth rounded-[10px] bg-gray-300 px-7 py-3"
          ...
          >
            {images.map((image, index) => (
              <div
                ...
                className="relative h-full w-31 shrink-0 cursor-pointer"
              >
                <div
                  className={cn(
                    'relative h-full w-full overflow-hidden rounded-sm border-2',
                    selectedImageIndex === index ? 'border-primary-200' : 'border-transparent',
                  )}
                >
                  <Image
                ...
                <button
                  ...
                  className="absolute -top-1 -right-1 flex h-5 w-5 cursor-pointer items-center justify-center rounded-full bg-gray-700 text-xs text-white"
                >
                  ×
                </button>
              </div>
            ...
            {images.length < 10 && (
              <button
                type="button"
                onClick={() => fileInputRef.current?.click()}
                className="ml-0.75 shrink-0 cursor-pointer"
              >
                <Image src="/icon/img-upload.svg" alt="Image upload icon" width={20} height={20} />
              </button>
            )}
          ...
          {showNext && (
            <button
              ...
              className="absolute top-1/2 right-2 z-10 flex h-6 w-6 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full bg-gray-50/50 backdrop-blur-sm"
            >
              <Image
                className="-rotate-90"
                src="/icon/drop-down.svg"
      ...
      <div className="divide-gray-450 border-gray-450 grid h-19 w-full grid-cols-3 divide-x rounded-[10px] border bg-gray-100">
        {/* 대표 이미지 지정 영역 */}
        <div
          className="flex items-center justify-between p-1.5"
          onDrop={(e) => handleRoleDrop(e, 'isRepresentative')}
          onDragOver={handleDragOver}
        >
          <div>
          ...
          <div className="relative aspect-square w-13 overflow-hidden rounded-md bg-gray-300">
            {representativeImage ? (
              <Image
                src={URL.createObjectURL(representativeImage.file)}
                alt="representative"
                fill
                className="object-cover"
              />
            ) : (
              <span className="typo-caption-regular flex w-full items-center justify-center p-1.5 text-center text-gray-500">
                위에서
                <br />
                선택
              </span>
        ...
        <div
          className="flex items-center justify-between p-1.5"
          onDrop={(e) => handleRoleDrop(e, 'isThumbnail1')}
          onDragOver={handleDragOver}
        >
          <div>
          ...
          <div className="relative aspect-square w-13 overflow-hidden rounded-md bg-gray-300">
            {thumbnail1Image ? (
              <Image
                src={URL.createObjectURL(thumbnail1Image.file)}
                alt="thumbnail1"
                fill
                className="object-cover"
              />
        ...
        <div
          className="flex items-center justify-between p-1.5"
          onDrop={(e) => handleRoleDrop(e, 'isThumbnail2')}
          onDragOver={handleDragOver}
        >
          <div>
          ...
          <div className="relative aspect-square w-13 overflow-hidden rounded-md bg-gray-300">
            {thumbnail2Image ? (
              <Image
                src={URL.createObjectURL(thumbnail2Image.file)}
                alt="thumbnail2"
                fill
                className="object-cover"
              />
